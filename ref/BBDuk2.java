 ...
 * Supercedes BBDukF by allowing simultaneous filtering, left- and
 * right-trimming with different references.
 * 
...
public class BBDuk2 {
	...
	public BBDuk2(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (Parser.parseZip(arg, a, b)) {
				...
				qfin2 = b;
			} else if (a.equals("out") || a.equals("out1") || a.equals("outu") || a.equals("outu1")
					|| a.equals("outnonmatch") ||
				...
				out2 = b;
			} else if (a.equals("outb") || a.equals("outm") || a.equals("outb1") || a.equals("outm1")
					|| a.equals("outbad") ||
				...
				assert (mink_ < 0 || (mink_ > 0 && mink_ < 32))
						: "kmin must be between 1 and 31; default is 4, negative numbers disable it.";
				...
				assert (hammingDistance >= 0 && hammingDistance <= 4)
						: "hamming distance must be between 0 and 4; default is 0.";
			} else if (a.equals("qhdist") || a.equals("queryhammingdistance")) {
				qHammingDistance = Integer.parseInt(b);
				assert (qHammingDistance >= 0 && qHammingDistance <= 4)
						: "hamming distance must be between 0 and 4; default is 0.";
			} else if (a.equals("edits") || a.equals("edist") || a.equals("editdistance")) {
				editDistance = Integer.parseInt(b);
				assert (editDistance >= 0 && editDistance <= 3)
						: "edit distance must be between 0 and 3; default is 0.";
			} else if (a.equals("hdist2") || a.equals("hammingdistance2")) {
				hammingDistance2 = Integer.parseInt(b);
				assert (hammingDistance2 >= 0 && hammingDistance2 <= 4)
						: "hamming distance must be between 0 and 3; default is 0.";
			} else if (a.equals("qhdist2") || a.equals("queryhammingdistance2")) {
				qHammingDistance2 = Integer.parseInt(b);
				assert (qHammingDistance2 >= 0 && qHammingDistance2 <= 4)
						: "hamming distance must be between 0 and 3; default is 0.";
			} else if (a.equals("edits2") || a.equals("edist2") || a.equals("editdistance2")) {
				editDistance2 = Integer.parseInt(b);
				assert (editDistance2 >= 0 && editDistance2 <= 3)
						: "edit distance must be between 0 and 2; default is 0.";
				...
				} // For some reason System.out does not print in verbose mode.
				...
				// else if(b.equalsIgnoreCase("right") ||
				// b.equalsIgnoreCase("r")){ktrimRight_=true;}
				// else if(b.equalsIgnoreCase("n") || b.equalsIgnoreCase("mask")){ktrimN_=true;}
				// else if(b.length()==1 && !b.equalsIgnoreCase("t") &&
				...
				// be f (false), l (left), r (right), or n");
				// }
				// }
				...
				assert (STATS_COLUMNS == 3 || STATS_COLUMNS == 5)
						: "statscolumns bust be either 3 or 5. Invalid value: " + STATS_COLUMNS;
		...
		if (hammingDistance2 == -1) {
			hammingDistance2 = hammingDistance;
		}
		if (qHammingDistance2 == -1) {
			qHammingDistance2 = qHammingDistance;
		}
		if (editDistance2 == -1) {
		...
		{// Process parser fields
			...
			// minTrimLength=(parser.minTrimLength>=0 ? parser.minTrimLength :
			// minTrimLength);
		...
		if (refFilter != null || literalFilter != null) {
			kfilter_ = true;
		}
		if (refMask != null || literalMask != null) {
			ktrimN_ = true;
		}
		if (refRight != null || literalRight != null) {
			ktrimRight_ = true;
		}
		if (refLeft != null || literalLeft != null) {
			ktrimLeft_ = true;
		}

		if (ref != null) {
			if (!ktrimN_ && !ktrimRight_ && !ktrimLeft_) {
				kfilter_ = true;
			}
		...
		if (refFilter != null) {
			for (String s : refFilter) {
				refNames.add(s);
			}
		}
		if (literalFilter != null) {
...

		{
			long usableMemory;
			long tableMemory;
			long numTables = Tools.max(1,
					(kfilter ? 1 : 0) + (ktrimLeft ? 1 : 0) + (ktrimRight ? 1 : 0) + (ktrimN ? 1 : 0));

			{
				long memory = Runtime.getRuntime().maxMemory();
				double xmsRatio = Shared.xmsRatio();
				usableMemory = (long) Tools.max(
						((memory - 96000000 - (20 * 400000 /* for atomic arrays */)) * (xmsRatio > 0.97 ? 0.82 : 0.72)),
						memory * 0.45);
			...
			if (initialSize < 1) {
				...
				initialSize = (prealloc_ || memOverWays < initialSizeDefault
						? (int) Tools.min(2142000000, (long) (mem2 / (12 * WAYS * numTables)))
						: initialSizeDefault);
		...
		if ((speed > 0 && qSkip > 1) || (qSkip > 1 && maxSkip > 1) || (speed > 0 && maxSkip > 1)) {
			System.err.println(
					"WARNING: It is not recommended to use more than one of 'qskip', 'speed', and 'rskip/maxskip' together.");
		...
		assert (findBestMatch == false || kfilter == false || kbig <= k)
				: "K must be less than 32 in 'findBestMatch' mode";
		...
		if (kfilter || ktrimN || ktrimRight || ktrimLeft) {
			...
			if (kbig > k) {
				System.err.println("kbig=" + kbig);
			}
			if (mink < k && (ktrimN || ktrimRight || ktrimLeft)) {
				System.err.println("mink=" + mink);
			}
			if (maskMiddle) {
				System.err.println("maskMiddle=" + maskMiddle);
			}
			if (hammingDistance > 0) {
				System.err.println("hamming distance=" + hammingDistance);
			}
			if (editDistance > 0) {
		...
		if (kfilter) {
			printFilterPlan("kfiltering", refFilter, literalFilter);
		}
		if (ktrimN) {
			printFilterPlan(((char) trimSymbol) + "-masking", refMask, literalMask);
		}
		if (ktrimRight) {
			printFilterPlan("right-ktrimming", refRight, literalRight);
		}
		if (ktrimLeft) {
			printFilterPlan("left-ktrimming", refLeft, literalLeft);
		}
		if (qtrimRight || qtrimLeft) {
			System.err.println("quality-trimming "
					+ ((qtrimRight && qtrimLeft) ? "both ends" : qtrimRight ? "right end" : " left end") + " to Q"
					+ trimq);
		}
		System.err.println();
		...
		if (in2 != null && (in2.contains("=") || in2.equalsIgnoreCase("null"))) {
			in2 = null;
		}
		if (in2 != null) {
			if (FASTQ.FORCE_INTERLEAVED) {
				System.err.println("Reset INTERLEAVED to false because paired input files were specified.");
			}
		...
		if (!Tools.testOutputFiles(overwrite, append, false, out1, out2, outb1, outb2, outsingle, outstats, outrpkm,
				outduk, outrqc, outrefstats)) {
		...
		if (!Tools.testForDuplicateFiles(true, in1, in2, qfin1, qfin2, out1, out2, outb1, outb2, outsingle, outstats,
				outrpkm, outduk, outrqc, outrefstats)) {
			throw new RuntimeException("\nSome file names were specified multiple times.\n");
		}

		assert (THREADS > 0) : "THREADS must be greater than 0.";

		assert (in1 == null || in1.toLowerCase().startsWith("stdin") || in1.toLowerCase().startsWith("standardin")
				|| new File(in1).exists()) : "Can't find " + in1;
		assert (in2 == null || in2.toLowerCase().startsWith("stdin") || in2.toLowerCase().startsWith("standardin")
				|| new File(in2).exists()) : "Can't find " + in2;

		if (!(kfilter || ktrimN || ktrimRight || ktrimLeft || qtrimLeft || qtrimRight || minAvgQuality > 0 || maxNs >= 0
				|| trimByOverlap ||
				...
				forceTrimLeft > 0 || forceTrimRight > 0 || forceTrimModulo > 0 || minBaseFrequency > 0
				|| recalibrateQuality)) {
			System.err.println(
		...
		if (ref != null) {
			for (String s0 : ref) {
				...
				assert (s == null || s.startsWith("stdin") || s.startsWith("standardin") || new File(s0).exists())
						: "Can't find " + s0;
			}
		}

		// Initialize tables
		final int tableType = (useForest ? AbstractKmerTable.FOREST1D
				: useTable ? AbstractKmerTable.TABLE : useArray ? AbstractKmerTable.ARRAY1D : 0);
	...
	private static void printFilterPlan(String action, String[] refs, String[] lits) {
		...
		System.err.println(action + " using " + (lits != null ? lits.length + " literal" + s1 : "") + and
				+ (refs != null ? refs.length + " reference" + s2 : "") + ".");
	}
	...
	public void process() {
		...
		if (!Tools.testOutputFiles(overwrite, append, false, out1, out2, outb1, outb2, outstats, outrpkm, outduk,
				outrqc, outrefstats)) {
			throw new RuntimeException(
					"One or more output files were duplicate or could not be written to.  Check the names or set the 'overwrite=true' flag.");
		}
	...
	public void process2(long startTime) {
		...
		{
			final boolean oldTI = FASTQ.TEST_INTERLEAVED; // TODO: This needs to be changed to a non-static field, or
															// somehow 'read mode' and 'ref mode' need to be
															// distinguished.
		...
		{
			long ram = freeMemory();
			ALLOW_LOCAL_ARRAYS = (scaffoldNames != null
					&& Tools.max(THREADS, 1) * 3 * 8 * scaffoldNames.size() < ram * 5);
		}
		...
		if (kfilter) {
			outstream.println("Contaminants:           \t" + readsKFiltered + " reads ("
					+ toPercent(readsKFiltered, readsIn) + ") \t" +
					basesKFiltered + " bases (" + toPercent(basesKFiltered, basesIn) + ")");
			outstream.flush();
		}
		if (qtrimLeft || qtrimRight) {
			outstream.println("QTrimmed:               \t" + readsQTrimmed + " reads ("
					+ toPercent(readsQTrimmed, readsIn) + ") \t" +
		...
		if (forceTrimLeft > 0 || forceTrimRight > 0 || forceTrimRight2 > 0 || forceTrimModulo > 0) {
			outstream.println("FTrimmed:               \t" + readsFTrimmed + " reads ("
					+ toPercent(readsFTrimmed, readsIn) + ") \t" +
					basesFTrimmed + " bases (" + toPercent(basesFTrimmed, basesIn) + ")");
		}
		if (ktrimLeft || ktrimRight || ktrimN) {
			outstream.println("KTrimmed:               \t" + readsKTrimmed + " reads ("
					+ toPercent(readsKTrimmed, readsIn) + ") \t" +
					basesKTrimmed + " bases (" + toPercent(basesKTrimmed, basesIn) + ")");
		}
		if (trimByOverlap) {
			outstream.println("Trimmed by overlap:     \t" + readsTrimmedByOverlap + " reads ("
					+ toPercent(readsTrimmedByOverlap, readsIn) + ") \t" +
					basesTrimmedByOverlap + " bases (" + toPercent(basesTrimmedByOverlap, basesIn) + ")");
		}
		if (filterGC) {
			outstream.println(
					"Filtered by GC:         \t" + badGcReads + " reads (" + toPercent(badGcReads, readsIn) + ") \t" +
		...
		if (minAvgQuality > 0 || maxNs >= 0 || minBaseFrequency > 0 || chastityFilter || removeBadBarcodes) {
			outstream.println("Low quality discards:   \t" + readsQFiltered + " reads ("
					+ toPercent(readsQFiltered, readsIn) + ") \t" +
					basesQFiltered + " bases (" + toPercent(basesQFiltered, basesIn) + ")");
		}
		if (calcEntropy) {
			outstream.println("Low entropy discards:   \t" + readsEFiltered + " reads ("
					+ toPercent(readsEFiltered, readsIn) + ") \t" +
		...
		outstream.println(
				"Total Removed:          \t" + readsRemoved + " reads (" + toPercent(readsRemoved, readsIn) + ") \t" +
	...
	public void unloadKmers() {
		for (AbstractKmerTable[] akta : new AbstractKmerTable[][] { filterMaps, maskMaps, trimRightMaps,
				trimLeftMaps }) {
			if (akta != null) {
				for (int i = 0; i < akta.length; i++) {
					akta[i] = null;
				}
			}
		}
		...
		if (STATS_COLUMNS == 3) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				tsw.print(String.format(Locale.ROOT, "%s\t%d\t%.5f%%\t%d\t%.5f%%\n", sn.name, sn.reads,
						(sn.reads * rmult), sn.bases, (sn.bases * bmult)));
			}
		}
		tsw.poisonAndWait();
	}
	...
	public static String rqcString() {
		...
		String[] keys = new String[] { "inputReads", "inputBases", "qtrimmedReads", "qtrimmedBases", "qfilteredReads",
				"qfilteredBases",
	...
	private static void putRqc(String key, long value, boolean evict) {
		putRqc(key, value + "", evict);
	}

	private static void putRqc(String key, String value, boolean evict) {
		if (RQC_MAP == null) {
			RQC_MAP = new HashMap<String, String>();
		}
		if (evict || !RQC_MAP.containsKey(key)) {
			RQC_MAP.put(key, value);
		}
	}
	...
	private String dukString(long time, String[] ref) {
		...
		sb.append("#Reference file:	"
				+ (ref == null || ref.length < 1 ? null : ref.length == 1 ? ref[0] : Arrays.toString(ref)) + "\n");
		...
		long size = 0;
		if (kfilter) {
			for (AbstractKmerTable x : filterMaps) {
				size += x.size();
			}
		}
		if (ktrimN) {
			for (AbstractKmerTable x : maskMaps) {
				size += x.size();
			}
		}
		if (ktrimRight) {
			for (AbstractKmerTable x : trimRightMaps) {
				size += x.size();
			}
		}
		if (ktrimLeft) {
			for (AbstractKmerTable x : trimLeftMaps) {
				size += x.size();
			}
		}
		...
		// prints a P value; not sure what it means
		sb.append("\n");
		...
		for (int i = 0; i < hitCounts.length; i++) {
			long x = hitCounts[i];
			if (x > 0) {
				sb.append(i).append('\t').append(x).append('\t').append(String.format(Locale.ROOT, "%.4f", (x * mult)))
						.append('\n');
			}
		}
	...
	private void toRefNames() {
		...
		for (int r = 0, s = 1; r < numRefs; r++) {
			...
			// System.err.println("r="+r+", s="+s+", scafs="+scafs+", lim="+lim+",
			// name="+name);
			while (s < lim) {
	...
	private long spawnLoadThreads(String[] ref, String[] literal, AbstractKmerTable[] maps, boolean countRef) {
		Timer t = new Timer();
		if ((ref == null || ref.length < 1) && (literal == null || literal.length < 1)) {
			return 0;
		}
		long added = 0;
		...
		if (ref != null) {
			for (String refname : ref) {
				...
				ConcurrentReadInputStream cris = ConcurrentReadInputStream.getReadInputStream(-1L, false, ff, null,
						null, null, Shared.USE_MPI, true);
				cris.start(); // 4567
				ListNum<Read> ln = cris.nextList();
				ArrayList<Read> reads = (ln != null ? ln.list : null);

				/* Iterate through read lists from the input stream */
				while (ln != null && reads != null && reads.size() > 0) {// ln!=null prevents a compiler potential null
																			// access warning
					{
		...
		if (literal != null) {
			...
			if (verbose) {
				System.err.println("Adding literals " + Arrays.toString(literal));
			}
		...
		if (!success) {
			KillSwitch.kill("Failed loading ref kmers; aborting.");
		}

		// Correct statistics for number of threads, since each thread processes all
		// reference data
		refKmers /= WAYS;
		refBases /= WAYS;
		refReads /= WAYS;
		...
		if (verbose) {
			...
			if (kfilter) {
				...
				for (AbstractKmerTable x : filterMaps) {
					x.dumpKmersAsText(tsw, k, 1, Integer.MAX_VALUE);
				}
			}
			if (ktrimN) {
	...
	private void spawnProcessThreads(Timer t) {
		...
		if (skipreads > 0) {
			...
			while (skipped < skipreads && reads != null && reads.size() > 0) {
				...
				if (ross != null) {
					ross.add(new ArrayList<Read>(1), ln.id);
				}
		...
		for (int i = 0; i < THREADS; i++) {
			alpt.add(new ProcessThread(cris, ros, rosb, ross, ALLOW_LOCAL_ARRAYS));
		}
		...
		for (ProcessThread pt : alpt) {
			...
			if (hitCounts != null) {
				for (int i = 0; i < hitCounts.length; i++) {
					hitCounts[i] += pt.hitCountsT[i];
				}
				pt.hitCountsT = null;
			}
			if (pt.scaffoldReadCountsT != null && scaffoldReadCounts != null) {
				for (int i = 0; i < pt.scaffoldReadCountsT.length; i++) {
					scaffoldReadCounts.addAndGet(i, pt.scaffoldReadCountsT[i]);
				}
				pt.scaffoldReadCountsT = null;
			}
			if (pt.scaffoldBaseCountsT != null && scaffoldBaseCounts != null) {
				for (int i = 0; i < pt.scaffoldBaseCountsT.length; i++) {
					scaffoldBaseCounts.addAndGet(i, pt.scaffoldBaseCountsT[i]);
				}
	...
	/**
	 * Loads kmers into a table. Each thread handles all kmers X such that
	 * X%WAYS==tnum.
	 */
	private class LoadThread extends Thread {
		...
		private long addToMap(Read r, int skip) {
			...
			if (skip > 1) { // Process while skipping some kmers
				for (int i = 0; i < bases.length; i++) {
					...
					if (verbose) {
						System.err.println("Scanning1 i=" + i + ", kmer=" + kmer + ", rkmer=" + rkmer + ", bases="
								+ new String(bases, Tools.max(0, i - k2), Tools.min(i + 1, k)));
					}
					if (len >= k) {
						refKmersT++;
						if (len % skip == 0) {
							...
							if (useShortKmers) {
								if (i == k2) {
									added += addToMapRightShift(kmer, rkmer, id);
								}
								if (i == bases.length - 1) {
									added += addToMapLeftShift(kmer, rkmer, extraBase, id);
								}
							}
						}
					}
				...
				for (int i = 0; i < bases.length; i++) {
					...
					if (verbose) {
						System.err.println("Scanning2 i=" + i + ", kmer=" + kmer + ", rkmer=" + rkmer + ", bases="
								+ new String(bases, Tools.max(0, i - k2), Tools.min(i + 1, k)));
					}
					if (len >= k) {
						refKmersT++;
		...
		private long addToMapLeftShift(long kmer, long rkmer, final long extraBase, final int id) {
			if (verbose) {
				System.err.println("addToMapLeftShift");
			}
			long added = 0;
			for (int i = k - 1; i >= mink; i--) {
				...
				if (verbose) {
					if ((toValue(kmer, rkmer, lengthMasks[i])) % WAYS == tnum) {
						System.err.println("added=" + x + "; i=" + i + "; tnum=" + tnum + "; Added left-shift kmer "
								+ AminoAcid.kmerToString(kmer & ~lengthMasks[i], i) + "; value="
								+ (toValue(kmer, rkmer, lengthMasks[i])) + "; kmer=" + kmer + "; rkmer=" + rkmer
								+ "; kmask=" + lengthMasks[i] + "; rightMasks[i+1]=" + rightMasks[i + 1]);
						System.err.println("i=" + i + "; tnum=" + tnum + "; Looking for left-shift kmer "
								+ AminoAcid.kmerToString(kmer & ~lengthMasks[i], i));
		...
		private long addToMapRightShift(long kmer, long rkmer, final int id) {
			if (verbose) {
				System.err.println("addToMapRightShift");
			}
			long added = 0;
			for (int i = k - 1; i >= mink; i--) {
				...
				// assert(Long.numberOfLeadingZeros(kmer)>=2*(32-i)) :
				// Long.numberOfLeadingZeros(kmer)+", "+i+", "+kmer+", "+kMasks[i];
				// assert(Long.numberOfLeadingZeros(rkmer)>=2*(32-i)) :
				// Long.numberOfLeadingZeros(rkmer)+", "+i+", "+rkmer+", "+kMasks[i];
				long x = addToMap(kmer, rkmer, i, extraBase, id, lengthMasks[i], hammingDistance2, editDistance2);
				added += x;
				if (verbose) {
					if ((toValue(kmer, rkmer, lengthMasks[i])) % WAYS == tnum) {
						System.err.println("added=" + x + "; i=" + i + "; tnum=" + tnum + "; Added right-shift kmer "
								+ AminoAcid.kmerToString(kmer & ~lengthMasks[i], i) + "; value="
								+ (toValue(kmer, rkmer, lengthMasks[i])) + "; kmer=" + kmer + "; rkmer=" + rkmer
								+ "; kmask=" + lengthMasks[i] + "; rightMasks[i+1]=" + rightMasks[i + 1]);
						System.err.println("i=" + i + "; tnum=" + tnum + "; Looking for right-shift kmer "
								+ AminoAcid.kmerToString(kmer & ~lengthMasks[i], i));
						final long value = toValue(kmer, rkmer, lengthMasks[i]);
						if (map.contains(value)) {
							System.err.println("Found " + value);
		...
		/**
		 * Adds this kmer to the table, including any mutations implied by editDistance
		 * or hammingDistance.
		 * 
		 * @param kmer      Forward kmer
		 * @param rkmer     Reverse kmer
		 ...
		 * @return Number of kmers stored
		 */
		private long addToMap(final long kmer, final long rkmer, final int len, final long extraBase, final int id,
				final long kmask0, final int hdist, final int edist) {

			assert (kmask0 == lengthMasks[len]) : kmask0 + ", " + len + ", " + lengthMasks[len] + ", "
					+ Long.numberOfTrailingZeros(kmask0) + ", " + Long.numberOfTrailingZeros(lengthMasks[len]);

			if (verbose) {
				System.err.println("addToMap_A; len=" + len + "; kMasks[len]=" + lengthMasks[len]);
			}
			assert ((kmer & kmask0) == 0);
			final long added;
			if (hdist == 0) {
				final long key = toValue(kmer, rkmer, kmask0);
				if (speed > 0 && ((key / WAYS) & 15) < speed) {
					return 0;
				}
				if (key % WAYS != tnum) {
					return 0;
				...
				if (verbose) {
					System.err.println(
							"addToMap_B: " + AminoAcid.kmerToString(kmer & ~lengthMasks[len], len) + " = " + key);
				}
			...
			if (verbose) {
				System.err.println("addToMap added " + added + " keys.");
			}
			return added;
		}
		 ...
		 * @return Number of kmers stored
		 */
		private long mutate(final long kmer, final long rkmer, final int len, final int id, final int dist,
				final long extraBase) {
			long added = 0;

			final long key = toValue(kmer, rkmer, lengthMasks[len]);

			if (verbose) {
				System.err.println("mutate_A; len=" + len + "; kmer=" + kmer + "; rkmer=" + rkmer + "; kMasks[len]="
						+ lengthMasks[len]);
			}
			if (key % WAYS == tnum) {
				if (verbose) {
					System.err.println(
							"mutate_B: " + AminoAcid.kmerToString(kmer & ~lengthMasks[len], len) + " = " + key);
				}
				int x = map.setIfNotPresent(key, id);
				if (verbose) {
					System.err.println("mutate_B added " + x + " keys.");
				}
				added += x;
	...
	/**
	 * Matches read kmers against reference kmers, performs binning and/or trimming,
	 * and writes output.
	 */
	private class ProcessThread extends Thread {
		 ...
		 */
		public ProcessThread(ConcurrentReadInputStream cris_, ConcurrentReadOutputStream ros_,
				ConcurrentReadOutputStream rosb_, ConcurrentReadOutputStream ross_, boolean localArrays) {
			cris = cris_;
			ros = ros_;
			rosb = rosb_;
			ross = ross_;
			...
			readstats = (MAKE_QUALITY_HISTOGRAM || MAKE_MATCH_HISTOGRAM || MAKE_BASE_HISTOGRAM || MAKE_QUALITY_ACCURACY
					||
					MAKE_EHIST || MAKE_INDELHIST || MAKE_LHIST || MAKE_GCHIST || MAKE_IDHIST) ? new ReadStats() : null;
		...
		public void run() {
			...
			while (ln != null && reads != null && reads.size() > 0) {// ln!=null prevents a compiler potential null
				...
				for (int i = 0; i < reads.size(); i++) {
					...
					if (r2 != null && !r2.validated()) {
						r2.validate(true);
					}

					if (readstats != null) {
						if (MAKE_QUALITY_HISTOGRAM) {
							readstats.addToQualityHistogram(r1);
						}
						if (MAKE_BASE_HISTOGRAM) {
							readstats.addToBaseHistogram(r1);
						}
						if (MAKE_MATCH_HISTOGRAM) {
							readstats.addToMatchHistogram(r1);
						}
						if (MAKE_QUALITY_ACCURACY) {
							readstats.addToQualityAccuracy(r1);
						}

						if (MAKE_EHIST) {
							readstats.addToErrorHistogram(r1);
						}
						if (MAKE_INDELHIST) {
							readstats.addToIndelHistogram(r1);
						}
						if (MAKE_LHIST) {
							readstats.addToLengthHistogram(r1);
						}
						if (MAKE_GCHIST) {
							readstats.addToGCHistogram(r1);
						}
						if (MAKE_IDHIST) {
							readstats.addToIdentityHistogram(r1);
						}
					}
					...
					if (verbose) {
						System.err.println("Considering read " + r1.id + " " + new String(r1.bases));
					}
					...
					if (removeBadBarcodes) {
						if (r1 != null && !r1.discarded() && r1.failsBarcode(barcodes, failIfNoBarcode)) {
							if (failBadBarcodes) {
								KillSwitch.kill("Invalid barcode detected: " + r1.id
					...
					boolean remove;
					if (removePairsIfEitherBad) {
						remove = r1.discarded() || (r2 != null && r2.discarded());
					} else {
						remove = r1.discarded() && (r2 == null || r2.discarded());
					}
					...
					if (remove) {
						...
						if (kfilter && storedKmersFilter > 0) {
							...
							if (!findBestMatch) {
								final int a = (kbig <= k ? countSetKmers(r1, filterMaps)
										: countSetKmersBig(r1, filterMaps));
								final int b = (kbig <= k ? countSetKmers(r2, filterMaps)
										: countSetKmersBig(r2, filterMaps));

								if (r1 != null && a > maxBadKmers) {
									r1.setDiscarded(true);
								}
								if (r2 != null && b > maxBadKmers) {
									r2.setDiscarded(true);
								}
							} else {
						...
						if (!remove && trimByOverlap && r2 != null && expectedErrors(r1, r2) < meeFilter) {
							...
							if (aprob == null || aprob.length < r1.length()) {
								aprob = new float[r1.length()];
							}
							if (bprob == null || bprob.length < r2.length()) {
								bprob = new float[r2.length()];
							}

							// Do overlap trimming
							r2.reverseComplement();
							// int bestInsert=BBMergeOverlapper.mateByOverlap(r1, r2, aprob, bprob,
							// overlapVector, minOverlap0, minOverlap,
							...
							int bestInsert = BBMergeOverlapper.mateByOverlapRatio(r1, r2, aprob, bprob, overlapVector,
									minOverlap0, minOverlap,
									minInsert0, minInsert, maxRatio, 0.12f, ratioMargin, ratioOffset, 0.95f, 0.95f,
									useQualityForOverlap);

							if (bestInsert < minInsert) {
								bestInsert = -1;
							}
							...
							if (bestInsert > 0 && !ambig) {
								if (bestInsert < r1.length()) {
									if (verbose) {
										System.err
												.println("Overlap right trimming r1 to " + 0 + ", " + (bestInsert - 1));
									}
									int x = TrimRead.trimToPosition(r1, 0, bestInsert - 1, 1);
									if (verbose) {
										System.err.println("Trimmed " + x + " bases: " + new String(r1.bases));
									}
								...
								if (bestInsert < r2.length()) {
									if (verbose) {
										System.err
												.println("Overlap right trimming r2 to " + 0 + ", " + (bestInsert - 1));
									}
									int x = TrimRead.trimToPosition(r2, 0, bestInsert - 1, 1);
									if (verbose) {
										System.err.println("Trimmed " + x + " bases: " + new String(r2.bases));
									}
									readsTrimmedByOverlapT++;
									basesTrimmedByOverlapT += x;
								}
					...
					if (!remove) {
						...
						if (minAvgQuality > 0) {
							if (r1 != null && r1.quality != null
									&& r1.avgQuality(false, minAvgQualityBases) < minAvgQuality) {
								r1.setDiscarded(true);
							}
							if (r2 != null && r2.quality != null
									&& r2.avgQuality(false, minAvgQualityBases) < minAvgQuality) {
								r2.setDiscarded(true);
							}
						}
						...
						if (minBaseFrequency > 0) {
							if (r1 != null && r1.minBaseCount() < minBaseFrequency * r1.length()) {
								r1.setDiscarded(true);
							}
							if (r2 != null && r2.minBaseCount() < minBaseFrequency * r2.length()) {
								r2.setDiscarded(true);
							}
						}
					...
					if (!remove && calcEntropy) {
						...
						if (r1 != null && !r1.discarded()
								&& entropyCutoff > averageEntropy(r1.bases, entropyK, entropyWindow,
										entropyCounts, entropyCountCounts, entropyKmerspace, verifyEntropy)) {
							r1.setDiscarded(true);
						}
						if (r2 != null && !r2.discarded()
								&& entropyCutoff > averageEntropy(r2.bases, entropyK, entropyWindow,
										entropyCounts, entropyCountCounts, entropyKmerspace, verifyEntropy)) {
							r2.setDiscarded(true);
						}
				...
				if (ros != null) {
					ros.add((removed > 0 ? Tools.condenseNew(reads) : reads), ln.id); // Creates a new list if old one
																						// became empty, to prevent
																						// shutting down the cris.
				}
		...
		private boolean ktrim0(final Read r1, final Read r2, ArrayList<Read> bad, AbstractKmerTable[] maps, int mode,
				int minlen1, int minlen2) {
		 ...
		 */
		private final int getValue(final long kmer, final long rkmer, final long lengthMask, final int qPos,
				final int len, final int qHDist, final AbstractKmerTable[] sets) {
		 ...
		 */
		private final int getValue(final long kmer, final long rkmer, final long lengthMask, final int qPos,
				final AbstractKmerTable[] sets) {
			assert (lengthMask == 0 || (kmer < lengthMask && rkmer < lengthMask))
					: lengthMask + ", " + kmer + ", " + rkmer;
			if (qSkip > 1 && (qPos % qSkip != 0)) {
				return -1;
			}
			...
			if (noAccel || ((key / WAYS) & 15) >= speed) {
				if (verbose) {
					System.err.println("Testing key " + key);
				}
		...
		private final int countSetKmers(final Read r, final AbstractKmerTable sets[]) {
			...
			for (int i = start; i < stop; i++) {
				...
				if (verbose) {
					System.err.println("Scanning6 i=" + i + ", kmer=" + kmer + ", rkmer=" + rkmer + ", bases="
							+ new String(bases, Tools.max(0, i - k2), Tools.min(i + 1, k)));
				}
				if (len >= minlen2 && i >= minlen) {
					final int id = getValue(kmer, rkmer, kmask, i, k, qHammingDistance, sets);
					if (verbose) {
						System.err.println("Testing kmer " + kmer + "; id=" + id);
					}
					if (id > 0) {
						if (verbose) {
							System.err.println("Found = " + (found + 1) + "/" + maxBadKmers);
						}
						if (found == maxBadKmers) {
							...
							} // Early exit, but prevents generation of histogram that goes over
								// maxBadKmers+1.
						}
						found++;
					}
			...
			if (hitCountsT != null) {
				hitCountsT[Tools.min(found, HITCOUNT_LEN)]++;
			}
			return found;
		}

		/**
		 * Returns the id of the sequence with the most kmer matches to this read, or -1
		 * if none are over maxBadKmers.
		 * 
		 * @param r    Read to process
		 * @param sets Kmer tables
			...
			for (int i = start; i < stop; i++) {
				...
				if (len >= minlen2 && i >= minlen) {
					...
					if (id > 0) {
						...
						found++;
						if (verbose) {
							System.err.println("Found = " + found + "/" + maxBadKmers);
						}
					}
				}
			}
		...
		private final int countSetKmersBig(final Read r, final AbstractKmerTable sets[]) {
			...
			for (int i = start; i < stop; i++) {
				...
				if (verbose) {
					System.err.println("Scanning7 i=" + i + ", kmer=" + kmer + ", rkmer=" + rkmer + ", bases="
							+ new String(bases, Tools.max(0, i - k2), Tools.min(i + 1, k)));
				}
				if (len >= minlen2 && i >= minlen) {
		...
		private final int ktrim(final Read r, final AbstractKmerTable[] sets, int mode) {
			...
			if (verbose) {
				System.err.println("KTrimming read " + r.id);
			}
			...
			for (int i = start; i < stop; i++) {
				...
				if (verbose) {
					System.err.println("Scanning3 i=" + i + ", kmer=" + kmer + ", rkmer=" + rkmer + ", len=" + len
							+ ", bases=" + new String(bases, Tools.max(0, i - k2), Tools.min(i + 1, k)));
				}
				if (len >= minlen2 && i >= minlen) {
					final int id = getValue(kmer, rkmer, kmask, i, k, qHammingDistance, sets);
					if (verbose) {
			...
			if (minLoc != minLocExclusive) {
				minLocExclusive = minLoc + k;
			}
			if (maxLoc != maxLocExclusive) {
				maxLocExclusive = maxLoc - k;
			}
			...
			if (useShortKmers && found == 0) {
				...
				if (mode == LEFTMODE || mode == NMODE) {
					...
					for (int i = start; i < lim; i++) {
						...
						len++;
						if (verbose) {
							System.err.println("Scanning4 i=" + i + ", kmer=" + kmer + ", rkmer=" + rkmer + ", bases="
									+ new String(bases, Tools.max(0, i - k2), Tools.min(i + 1, k)));
						}
						if (len >= mink) {
				...
				if (mode == RIGHTMODE || mode == NMODE) {
					...
					for (int i = stop - 1; i > lim; i--) {
						...
						len++;
						if (verbose) {
							System.err.println("Scanning5 i=" + i + ", kmer=" + kmer + ", rkmer=" + rkmer + ", bases="
									+ new String(bases, Tools.max(0, i - k2), Tools.min(i + 1, k)));
						}
						if (len >= mink) {
							...
							if (verbose) {
								System.err.println("Looking for right kmer "
										+ AminoAcid.kmerToString(kmer & ~lengthMasks[len], len) + "; id=" + id
										+ "; kmask=" + lengthMasks[len]);
							}
							if (id > 0) {
								if (id0 < 0) {
									id0 = id;
			...
			if (verbose) {
				System.err.println("found=" + found + ", minLoc=" + minLoc + ", maxLoc=" + maxLoc + ", minLocExclusive="
						+ minLocExclusive + ", maxLocExclusive=" + maxLocExclusive);
			}
			...
			if (mode == LEFTMODE) { // Trim from the read start to the rightmost kmer base
				if (verbose) {
					System.err.println(
							"Left trimming to " + (ktrimExclusive ? maxLocExclusive + 1 : maxLoc + 1) + ", " + 0);
				}
				int x = TrimRead.trimToPosition(r, ktrimExclusive ? maxLocExclusive + 1 : maxLoc + 1, bases.length - 1,
						1);
				if (verbose) {
					System.err.println("Trimmed " + x + " bases: " + new String(r.bases));
				}
				return x;
				...
				if (verbose) {
					System.err.println(
							"Right trimming to " + 0 + ", " + (ktrimExclusive ? minLocExclusive - 1 : minLoc - 1));
				}
		...
		private final int kmask(final Read r, final AbstractKmerTable[] sets) {
			...
			if (verbose) {
				System.err.println("KMasking read " + r.id);
			}
			final byte[] bases = r.bases, quals = r.quality;
			if (bases == null || bases.length < k) {
				return 0;
				...
				if (len >= minlen2 && i >= minlen) {
					...
					if (id > 0) {
						...
						if (verbose) {
							...
							System.err.println("i=" + i + ", minus=" + minus + ", plus=" + plus + ", trimpad=" + trimPad
									+ ", k=" + k);
						}
			...
			if (verbose) {
				System.err.println("found=" + found + ", bitset=" + bs);
			}
		...
		private float averageEntropy(final byte[] bases, final int k,
				final int window, final short[] counts, final short[] countCounts, final int kmerspace,
				boolean verify) {
			...
			for (int i = 0, i2 = -window; i2 < bases.length; i++, i2++) {
				...
				if (i < bases.length) {
					...
					countCounts[counts[kmer]]++;
					if (verify) {
						assert (current == Tools.cardinality(counts))
								: current + ", " + Tools.cardinality(counts) + "\n" + Arrays.toString(counts);
						assert (Tools.sum(countCounts) > 0 && (Tools.sum(countCounts) <= window + 1))
								: current + ", " + Tools.cardinality(counts) + "\n" + Arrays.toString(countCounts);
					}
				...
				if (i2 >= 0) {
					...
					if (verify) {
						assert (current == Tools.cardinality(counts))
								: current + ", " + Tools.cardinality(counts) + "\n" + Arrays.toString(counts);
						assert (Tools.sum(countCounts) >= 0 && (Tools.sum(countCounts) <= window))
								: current + ", " + Tools.cardinality(counts) + "\n" + Arrays.toString(countCounts);
					}
				...
				if (verify && i2 > -1 && i < bases.length) {
					...
					assert (Tools.sum(countCounts) == window)
							: current + ", " + Tools.cardinality(counts) + "\n" + Arrays.toString(countCounts);
				}
			...
			// System.err.println(" *** ");
			// System.err.println(entropySum+", "+entropyMeasurements+",
			// "+(entropySum/(Tools.max(1, entropyMeasurements))));
			// System.err.println(window+", "+k+", "+kmerspace+", "+counts.length+",
			// "+countCounts.length);
			// System.err.println(" *** ");
	...
	/**
	 * Transforms a kmer into a canonical value stored in the table. Expected to be
	 * inlined.
	 * 
	 * @param kmer       Forward kmer
	 * @param rkmer      Reverse kmer
	...
	/**
	 * Hold kmers for filtering. A kmer X such that X%WAYS=Y will be stored in
	 * keySets[Y]
	 */
	...
	/**
	 * scaffoldCounts[id] stores the number of reads with kmer matches to that
	 * scaffold
	 */
	private AtomicLongArray scaffoldReadCounts;
	/**
	 * scaffoldBaseCounts[id] stores the number of bases with kmer matches to that
	 * scaffold
	 */
	...
	/**
	 * Maximum input reads (or pairs) to process. Does not apply to references. -1
	 * means unlimited.
	 */
	...
	/**
	 * Attempt to match kmers shorter than normal k on read ends when doing
	 * kTrimming.
	 */
	...
	final int mink;
	/**
	 * A read may contain up to this many kmers before being considered a match.
	 * Default: 0
	 */
	...
	/**
	 * If positive, calculate average quality from the first X bases only. Default:
	 * 0
	 */
	...
	/**
	 * Trim implied adapters based on overlap, for reads with insert size shorter
	 * than read length
	 */
	...
	/**
	 * Pairs go to outbad if either of them is bad, as opposed to requiring both to
	 * be bad.
	 * Default: true.
	 */
